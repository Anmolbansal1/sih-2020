...
class User(db.Model):
    ...
    exp_date = db.Column('exp_date', db.DateTime, nullable=False)
...
def record():
    ...
    gait_label,probs_gait,label_enc_gait, type_ = inst.predict(np.asarray(final_img))
    print('&'*100)
    ...
    final_model.get_gait_classes(label_enc_gait, type_)
...
def getFace():
    ...
    face_pred,face_probs,class_list,num_img,type_= fm.get_output()
    
    final_model.get_face_probs(face_probs)
    print(face_probs)

    final_model.get_face_classes(class_list, type_)
...
def final_model_app():
    ...
    type_ = final_model.type_
    return {'final_answer':str(final_pred), 'final_prob': final_prob, 'face_prob': face_probs, 'gait_prob': gait_probs, 'type': type_}
...
def grant():
    ...
    if name=="terrorist":
        inst.save(frames=final_model.gait_frames,label="Unknown", type_="PERM")
        # cv2.imshow("kk",final_model.gait_frames[0])
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        fm.save('Unknown', "PERM")
    else:
        fm.add_class(label=name, type_=option)
        # cv2.imshow("kk",final_model.gait_frames[0])
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        inst.add_user(final_model.gait_frames_center,name, type_=option)
        inst.save(frames=final_model.gait_frames,label=name,frame_human=final_model.gait_frames_center,type_=option)
        fm.save(name, option)
    fm.reset()
